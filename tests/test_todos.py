...
def test_list_todos_should_return_5__todos(session, client, user, token):
    expected_todos = 5
...
def test_list_todos_filter_title_should_return_5_todos(
    ...
    session.bulk_save_objects(
        TodoFactory.create_batch(5, user_id=user.id, title='Test todo 2')
    )
    session.commit()
...
def test_list_todos_filter_description_should_return_5_todos(
    ...
    session.bulk_save_objects(
        TodoFactory.create_batch(5, user_id=user.id, description='text')
    )
    session.commit()
